...
class Borg:
    ...
    def __init__(self, numberOfVariables, numberOfObjectives, numberOfConstraints, function, epsilons=None, bounds=None, directions=None, seed=None):
        ...
        directions          - The optimization direction (min or max) for each objective. We encourage the default of None (minimization).
        ...
        if directions is None:
            ...
            if directions == "min":
                directions = None #[False]*numberOfObjectives
            elif directions == "max":
                directions = [True]*numberOfObjectives
        elif isinstance(directions, list):
            #(True for maximization, False for minimization)
            directions = [True if d == "max" else False for d in directions]
        else:
            print("Invalid directions argument.  Must be None or a string (min or max) or a list of strings.")
    ...
    def solveMPI(self, islands=1, maxTime=None, maxEvaluations=None, initialization=None, runtime=None,
            allEvaluations=None, frequency=None, newCheckpointFileBase=None, oldCheckpointFile=None, evaluationFile=None):
        ...
        if runtime:
            if isinstance(runtime, bytes) is False:
                runtime_c = bytes(str(runtime), 'utf-8')
            else:
                runtime_c = runtime
            Configuration.libborg.BORG_Algorithm_output_runtime(runtime_c);
        ...
        if newCheckpointFileBase:
            if isinstance(newCheckpointFileBase, bytes) is False:
                newCheckpointFileBase_c = bytes(str(newCheckpointFileBase), 'utf-8')
            else:
                newCheckpointFileBase_c = newCheckpointFileBase
            Configuration.libborg.BORG_Algorithm_new_checkpoint_base(newCheckpointFileBase_c)

        if oldCheckpointFile:
            if isinstance(oldCheckpointFile, bytes) is False:
                oldCheckpointFile_c = bytes(str(oldCheckpointFile), 'utf-8')
            else:
                oldCheckpointFile_c = oldCheckpointFile
            Configuration.libborg.BORG_Algorithm_old_checkpoint(oldCheckpointFile_c)

        if evaluationFile:
            if isinstance(evaluationFile, bytes) is False:
                evaluationFile_c = bytes(str(evaluationFile), 'utf-8')
            else:
                evaluationFile_c = evaluationFile
            Configuration.libborg.BORG_Algorithm_output_evaluations(evaluationFile_c)
    ...
    def solve(self, settings={}):
        ...
        settings - Dictionary of parameters for the Borg MOEA.  The key should match one
            ...
            are used for any undefined parameters. settings can also include the following keys:

                maxEvaluations        - The maximum number of function evaluations (default: 10000)
                initialPopulationSize - The initial population size (default: 100)
                minimumPopulationSize - The minimum population size (default: 100)
                maximumPopulationSize - The maximum population size (default: 10000)

                runtimefile           - The file to save runtime dynamics (default: None)
                runtimeformat         - The format for runtime dynamics (default: 'optimizedv')
                frequency             - The frequency of saving runtime dynamics (default: None)
                data_header           - The header for data in runtime dynamics (default: None)

                injectionRate         - The injection rate (default: 0.25)
                selectionRatio        - The selection ratio (default: 0.02)
                restartMode           - The restart mode (default: RestartMode.DEFAULT)
                maxMutationIndex      - The maximum mutation index (default: 10)
                probabilityMode       - The probability mode (default: ProbabilityMode.DEFAULT)
                pm.rate               - The probability of mutation (default: 1/numberOfVariables)
                pm.distributionIndex  - The distribution index for mutation (default: 20.0)
                sbx.rate              - The probability of crossover (default: 1.0)
                sbx.distributionIndex - The distribution index for crossover (default: 15.0)
                de.crossoverRate      - The crossover rate for DE (default: 0.1)
                de.stepSize           - The step size for DE (default: 0.5)
                um.rate               - The probability of mutation (default: 1/numberOfVariables)
                spx.parents           - The number of parents for SPX (default: 10)
                spx.offspring         - The number of offspring for SPX (default: 2)
                spx.epsilon           - The epsilon value for SPX (default: 3.0)
                pcx.parents           - The number of parents for PCX (default: 10)
                pcx.offspring         - The number of offspring for PCX (default: 2)
                pcx.eta               - The eta value for PCX (default: 0.1)
                pcx.zeta              - The zeta value for PCX (default: 0.1)
                undx.parents          - The number of parents for UNDX (default: 10)
                undx.offspring        - The number of offspring for UNDX (default: 2)
                undx.zeta             - The zeta value for UNDX (default: 0.5)
                undx.eta              - The eta value for UNDX (default: 0.35
        """
        ...
        if settings.get("oldCheckpointFile") is not None:
            oldCheckpointFile = settings['oldCheckpointFile']
            if isinstance(oldCheckpointFile, bytes) is False:
                oldCheckpointFile_c = bytes(str(oldCheckpointFile), 'utf-8')
            else:
                oldCheckpointFile_c = oldCheckpointFile
            Configuration.libborg.BORG_Algorithm_old_checkpoint(oldCheckpointFile_c)
            Configuration.libborg.BORG_Algorithm_restore(algorithm)
        ...
        while Configuration.libborg.BORG_Algorithm_get_nfe(algorithm) < maxEvaluations:
            ...
            if statistics is not None and currentEvaluations-lastSnapshot >= frequency:
                ...
                #### alh: create new checkpoint file
                if settings.get("newCheckpointFileBase") is not None:
                    newCheckpointFileBase = settings['newCheckpointFileBase']
                    if isinstance(newCheckpointFileBase, bytes) is False:
                        newCheckpointFileBase_c = bytes(str(newCheckpointFileBase), 'utf-8')
                    else:
                        newCheckpointFileBase_c = newCheckpointFileBase
                    Configuration.libborg.BORG_Algorithm_new_checkpoint_base(newCheckpointFileBase_c)
                    #Configuration.libborg.BORG_Algorithm_new_checkpoint(newCheckpointFileBase_c)
                    Configuration.libborg.BORG_Algorithm_checkpoint(algorithm)
                    
        result = Configuration.libborg.BORG_Algorithm_get_result(algorithm)

        if settings.get("runtimefile") is not None:
            fp.close()
...
def _functionWrapper(function, numberOfVariables, numberOfObjectives, numberOfConstraints, directions=None):
    ...
    directions - The array of optimization directions #(True for maximization, False for minimization)
    """
