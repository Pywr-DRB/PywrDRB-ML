...
ml_model = WaterTempRandomForestUncertaintyModel(
    model1=pn.get() / f"models/{folder}/rf_model1.gz",
    model2=pn.get() / f"models/{folder}/rf_model2.gz",
    model_map=pn.get() / f"models/{folder}/rf_model_map.gz",
    debug=True,
    thermal_mitigation_bank_size=1620,  # mgd
    )
ml_model.load_data(database, start_date='1979-01-01', end_date='2023-12-31')
