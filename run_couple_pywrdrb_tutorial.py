...
pn.chdir()
pn.mkdir("outputs/coupled_pywrdrb_tutorial")
output_folder = "coupled_pywrdrb_tutorial"
import pywrdrb

#%% Create coupled Pywr-DRB model
inflow_type = 'pub_nhmv10_BC_withObsScaled'
model_filename = str(pn.outputs.get(output_folder) / f"{inflow_type}.json")
output_filename = str(pn.outputs.get(output_folder) / f"{inflow_type}.hdf5")
