...
def plot_parallel_coords_with_kde(
    ...
    ax.legend(
        ...
        bbox_to_anchor=(0.7, 0.5),  # x > 1 pushes farther right; y=0.5 centers vertically
        fontsize=9
    )
...
release_names = ['Rule-based'] + names[3:5] + ['historic']  # All release types you want to plot
n_release_types = len(release_names)

fig = plt.figure(figsize=(6, 4.5))
gs = gridspec.GridSpec(8, 1, height_ratios=[0.5, 0.15, 0.3, 0.3, 0.3, 0.3, 0.3, 0.3], hspace=0)

# Top subplot: Tmax
ax1 = fig.add_subplot(gs[0])
ax1.plot(df_["Tmax (No control)"], ls="-", color=colors['No control'], label="No control")
ax1.plot(df_["Tmax (Rule-based)"], ls="-", color=colors['Rule-based'], label="Fixed release")
#ax1.plot(df_["Tmax (historic)"], ls="-", color=colors['historic'], label="Tmax (historic)")
# for rbf_name in names[3:5]:
#     ax1.plot(df_[f"Tmax ({rbf_name})"], ls="-", color=colors[f"{rbf_name}"], label=f"Tmax ({rbf_name})")
ax1.plot(df_[f"Tmax ({rbf_name})"], ls="-", color=colors[f"{names[3]}"], label="Standard bank,\nmax Jrel")
ax1.plot(df_[f"Tmax ({rbf_name})"], ls="-", color=colors[f"{names[4]}"], label="Standard bank,\nmax Jadd")
ax1.axhline(24, lw=1, c="k", ls=":")
ax1.set_ylabel("$T_{max}$ (°C)")
ax1.grid(True, axis='y', lw=0.3, ls="--")
#ax1.tick_params(axis='x', direction='in')
...
new_labels = [lbl.replace("Tmax ", "") for lbl in labels]
ax1.legend(
    handles,
    new_labels,
    frameon=False,
...
for i, release_name in enumerate(release_names):
    ...
    if release_name == 'historic':
        ...
        if release_name == 'Rule-based':
            ax.bar(df_.index, df_[release_name], width=1.0, color=colors[release_name], 
                   label="Fixed release\n(baseline)", alpha=0.6, zorder=4)
        if release_name == 'RBF-better Jrel':
            ax.bar(df_.index, df_[release_name], width=1.0, color=colors[release_name], 
                   label="Standard bank,\nmax Jrel", alpha=0.6, zorder=4)
        if release_name == 'RBF-better Jadd':
            ax.bar(df_.index, df_[release_name], width=1.0, color=colors[release_name], 
                   label="Standard bank,\nmax Jadd", alpha=0.6, zorder=4)
    ax.grid(True, axis='y', lw=0.3, ls="--")
    #ax.set_ylabel(f"{release_name}\nThermal release (mgd)")
    ax.set_ylim([0, 130])
    ax.legend(frameon=False, loc='center left', bbox_to_anchor=(1, 0.5))
...
plt.tight_layout()
pn.outputs.mkdir(f"dps_{policy}_{job_id}/figures/RBFs")
clt.fig.savefig(fig, pn.figures.get("attemp1") / f"RBFs_thermal_release_{yr}_poster.jpg")
#clt.fig.savefig(fig, pn.figures.get(f"attemp1") / f"RBFs_thermal_release_{yr}_with_hist.jpg")
###clt.fig.savefig(fig, pn.outputs.get(f"dps_{policy}_{job_id}/figures/RBFs") / f"RBFs_{yr}.jpg")
plt.show()
