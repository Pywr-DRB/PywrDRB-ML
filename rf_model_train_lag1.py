...
df = database[["tmmn", "tmmx", "pr", "srad", "bc_cannonsville_storage_pct", "doy", "QbcTavg_Q_C", "QbcTavg_T_C", "tavg_water_cannonsville_src", "QbcTavg_T_C_lag1"]].dropna()
df.loc[df["tavg_water_cannonsville_src"] != "obs", "QbcTavg_T_C"] = np.nan
df.dropna(axis=0, how='any', inplace=True)
x_vars = ["tmmn", "tmmx", "pr", "srad", "bc_cannonsville_storage_pct", "QbcTavg_Q_C", "doy", "QbcTavg_T_C_lag1"]
y_var = "QbcTavg_T_C"
X = df[x_vars].values
obs = df[y_var].values
...
rf_model1.save(pn.models.get(folder) / "rf_model1_lag1.gz")
...
df = database[["tmmn", "tmmx", "pr", "srad", "QbcTavg_Q_i", "doy", "QbcTavg_Q_C", "QbcTavg_T_i", "tavg_water_src", "QbcTavg_T_i_lag1"]].dropna()
df.loc[df["tavg_water_src"] != "obs", "QbcTavg_T_i"] = np.nan
df.dropna(axis=0, how='any', inplace=True)
x_vars = ["tmmn", "tmmx", "pr", "srad", "QbcTavg_Q_i", "QbcTavg_Q_C", "doy", "QbcTavg_T_i_lag1"]
y_var = "QbcTavg_T_i"
X = df[x_vars].values
obs = df[y_var].values
...
rf_model2.save(pn.models.get(folder) / "rf_model2_lag1.gz")
...
X = df[x_vars].values
obs = df[y_var].values
rf_model_map = RandomForestUncertaintyModel.load(pn.models.get(folder) / "rf_model_map.gz")
...
df_res.to_csv(pn.models.get(folder) / "simple_run_Temp_lag1.csv")
...
for y in range(1979, 2024):
    ...
    plt.tight_layout()
    clt.fig.savefig(fig, filename=pn.models.get(f"{folder}/figures") / f"temp_{y}_lag1.png")
    plt.show()
...
df = database[["Q_Trenton_bc", "Q_Schuylkill_bc", "Q_Trenton_bc_7d_avg", "Q_Schuylkill_bc_7d_avg", "doy", "saltfront", "saltfront_src", "saltfront_lag1"]].dropna()
df.loc[df["saltfront_src"] != "obs", "saltfront"] = np.nan
df.dropna(axis=0, how='any', inplace=True)
x_vars = ["Q_Trenton_bc", "Q_Schuylkill_bc", "Q_Trenton_bc_7d_avg", "Q_Schuylkill_bc_7d_avg", "doy", "saltfront_lag1"]
y_var = "saltfront"
X = df[x_vars].values
obs = df[y_var].values
...
rf_model_saltfront.save(pn.models.get(folder) / "rf_model_saltfront_lag1.gz")
...
df_res.to_csv(pn.models.get(folder) / "simple_run_saltfront_lag1.csv")

# RMSE on training set: 1.316
# Mean RMSE from cross-validation: 3.654
#%% Plot saltfront
pn.mkdir(pn.models.get(folder) / "figures")
...
for y in range(1979, 2024):
    ...
    plt.tight_layout()
    clt.fig.savefig(fig, filename=pn.models.get(f"{folder}/figures") / f"salt_front_{y}_lag1.png")
    plt.show()
