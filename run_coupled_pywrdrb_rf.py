...
pn.chdir()
pn.mkdir("outputs/coupled_pywrdrb_rf_asycronized")
pn.sc.add("wd", pn.get("outputs/coupled_pywrdrb_rf_asycronized"), overwrite=True)

import pywrdrb

#%% Create coupled Pywr-DRB model
inflow_type = 'pub_nhmv10_BC_withObsScaled'
model_filename = str(pn.sc.wd / f"{inflow_type}_asycronized.json")
output_filename = str(pn.sc.wd / f"{inflow_type}_asycronized.hdf5")
...
#%% Asycronized post analysis (super fast)
temperature_model = model.parameters["temperature_model"]
ml_model = temperature_model.ml_model
...
df_temperature = data.temperature['pub_nhmv10_BC_withObsScaled_asycronized'][0]
